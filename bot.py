...
Также оповестите пользователя, что при желании он может просто указать нужные координаты адреса на карте через соответствующую кнопку в меню чата справа снизу.
...
chat_id текущего пользователя - {self.chat_id}"""
                ...
                try:        
                    ...
                    if (
                        "заявка" in output.lower() and ("создан" in output.lower() or "оформл" in output.lower())
                    ) and (
                        (
