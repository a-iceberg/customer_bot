    ...
    def setup_routes(self):
        ...
        async def handle_message(request: Request, authorization: str = Header(None)):
            ...
            if user_message == "/fullreset":
                ...
                system_prompt = f"""Текущие дата и время - {now}. Вы - сотрудник колл-центра сервисного центра по ремонту бытовой техники. Вы получаете сообщения от пользователя c аккаунта с именем Ластик67.
...
если имеющееся у вас выше имя аккаунта не выглядит, как обычное человеческое, а как какой-то логин / никнейм, - в НАЧАЛЕ диалога до всех остальных вопросов однократно запрашивайте имя пользователя, как к нему можно обращаться. Но пользователь может отказаться называть это, в таком случае снова НЕ настаивайте;
