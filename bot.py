...
Сейчас вы получили от пользователя с chat_id {self.chat_id} следующее сообщение: {user_message}"""
                ...
                bot_response = await self.chat_agent.agent.arun(
                    input=message_text, chat_history=chat_history, chat_id=self.chat_id
                )
                await self.chat_history_service.save_to_chat_history(
                    self.chat_id,
                    user_message,
