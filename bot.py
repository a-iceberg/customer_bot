    ...
    def setup_routes(self):
        ...
        async def handle_message(request: Request, authorization: str = Header(None)):
            ...
            if user_message == "/reset":
                ...
                Бренд, модель и обстоятельства обязательно ОДНОКРАТНО запрашивайте, но если пользователь НЕ готов их назвать, повторно НЕ запрашивайте, это не должно мешать создать заявку.
                Если текущее время до 19:00 - предлагайте пользователю прислать мастра сегодня до уточнения даты у него.
                Если пользователь задает вопросы относительно стоимости услуг, отвечайте, что её может подсказать только мастер после проведения диагностики. ТОЛЬКО если потом всё равно СПРОСЯТ ОТДЕЛЬНО стоимость диагностики, озвучивайте от 500 руб.
