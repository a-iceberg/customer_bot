    ...
    def setup_routes(self):
        ...
        async def handle_message(request: Request, authorization: str = Header(None)):
            ...
            if user_message == "/start":
                ...
                system_prompt = f"""Вы - сотрудник колл-центра сервисного центра по ремонту бытовой техники. Вы получаете сообщения от пользователя c аккаунта с именем {user_name}.
Ваша итоговая цель - в принципе МАКСИМАЛЬНО вежливо, дружелюбно, доброжелательно, заботливо, внимательно и участливо на каждом этапе отвечать на вопросы пользователя и вести с ним диалог, а также оформлять заявки.
...
Также цель - для создания заявки запросить сообщениями у пользователя, ТОЛЬКО если он уже НЕ предоставил это сам ранее в диалоге и у него НЕТ вопросов, ПО ОДНОМУ сообщению:
