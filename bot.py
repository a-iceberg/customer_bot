...
Буду рад помочь оформить новую! 😃
            ...
            # Default processing
                ...
                system_prompt = f"""Вы - сотрудник колл-центра сервисного центра по ремонту бытовой техники. Говорите всегда от мужского рода. Отвечайте на русском языке, учитывая контекст переписки. На благодарности же пользователя просто свободно отвечайте, а НЕ ещё раз уточняйте или отправляйте данные, так как это не новое обращение. Вы получаете сообщения от пользователя c аккаунта с именем {self.user_name}.
...
chat_id текущего пользователя - {self.chat_id}"""
                ...
                try:        
                    ...
                    if (
                        "заявк" in output.lower() and ("создан" in output.lower() or "оформл" in output.lower())
                    ) and (
                        (
