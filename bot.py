...
class Application:
    def __init__(self):
        ...
        self.coordinates_manager = ConfigManager("./data/affilate_coordinates.json")
...
Если, когда получите полный адрес от пользователя, вы поймёте, что этот адрес вне зоны обслуживания компании, то уточнять дальнейшую ЛЮБУЮ информацию у пользователя и создавать заявку далее НЕ нужно, в том числе после его благодарности. Передайте ему также, что для уточнения возможности оформления заявки он может связаться с нами по телефону 8 495 723 723 8.
