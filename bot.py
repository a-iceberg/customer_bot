...
Cвяжитесь с нами по телефону 8 495 463 50 46 для дальнейшей помощи.
        ...
        self.llm_error_answer = """
            Извините, произошла ошибка в работе системы.
Попробуйте сформулировать ваше сообщение по-другому или же свяжитесь с нами по телефону 8 495 463 50 46 для дальнейшей помощи.
        """
    ...
    def setup_routes(self):
        ...
        async def handle_callback_query(
            ...
            if callback['data'].startswith('request_'):
                ...
                system_prompt = f"""Вы - только сотрудник колл-центра сервисного центра по ремонту и различным бытовым услугам. Говорите всегда от мужского рода. Отвечайте на русском языке, учитывая контекст переписки. На благодарности же пользователя просто свободно отвечайте, а НЕ ещё раз уточняйте или отправляйте данные, так как это не новое обращение.
...
Вы получаете сообщения от пользователя c аккаунта с именем {user_name}.
