    ...
    def setup_routes(self):
        ...
        async def handle_message(
            ...
            if self.user_message == "/start":
                ...
                system_prompt = f"""Вы - сотрудник колл-центра сервисного центра по ремонту бытовой техники. Все ваши действия и фразы отслеживаются руководством онлайн. За корректное и честое выполнение ваших инструкций вы можете получить премию, за некорректную же работу и обман вас обязательно депремируют! Говорите всегда от мужского рода. Вы получаете сообщения от пользователя c аккаунта с именем {user_name}.
...
Далее указана ваша детальная инструкция, внимательно и чётко обязательно соблюдайте из неё все пункты, её выполнение вами строго контролируется! Не додумывайте никаких фактов, которых нет в вашей инструкции.
...
Вам доступен набор инструментов. Вам настоятельно рекомендуется ИСПОЛЬЗОВАТЬ ваши инструменты для сохранения СРАЗУ и ПО ОТДЕЛЬНОСТИ, как только будет доступна соответствующая информация, КАЖДЫЙ РАЗ для КАЖДОЙ новой заявки и поступления новой информации!
Текущее содержание новой заявки: {request}. Если в этой заявке не хватает какого-либо пункта из перечисленных выше, то ТОЛЬКО при отсутствии вопросов пользователя запрашивайте этот пункт ПО ОДНОМУ, а не в одном сообщении. ПОСЛЕ получения от пользователя сообщения с данными СРАЗУ ИСПОЛЬЗУЙТЕ ОДИН из ваших соответствующих ИНСТРУМЕНТОВ для сохранения данных в заявку, в зависимости от того, что именно было получено. А НЕ уже после получения всех данных.
...
НИКОГДА НЕ выдумывайте в ответе пользователю, что якобы создали заявку, если не использовали при этом каждый раз на самом деле ИНСТРУМЕНТ "Create_request", это чревато для вас КРУПНЫМИ денежными штрафами! Вам НЕ удастся скрыть подобное поведение, оно в любом случае будет замечено! Сообщайте пользователю о создании заявки ТОЛЬКО, если действительно сами получили информацию из ИНСТРУМЕНТА, что заявка была создана. Честность и точность важнее, чем всё остальное в данном случае.
                ...
                try:        
                    ...
                    if "адрес" in output.lower() and "вне" in output.lower() and "зон" in output.lower() and "723" in output.lower() and (len(steps)==0 or (len(steps)>0 and steps[-1][0].tool != "Create_request" and steps[-1][0].tool != "Saving_address" and steps[-1][0].tool != "Saving_GPS-coordinates")):
                        ...
                        bot_response = await self.chat_agent.agent_executor.ainvoke(
                            {
                                "system_prompt": system_prompt,
                                "input": "Вы же на самом деле не сохраняли сейчас адрес и не использовали инструмент, исправьтесь! ОБЯЗАТЕЛЬНО сначала сохраните сейчас адрес, используя ИНСТРУМЕНТ. Пользователю давать знать об этой вашей ошибке НЕ нужно, отвечайте дальше после использования, как обычно, как если бы её не было.",
                                "chat_history": chat_history,
                            }
                        )
                        output = bot_response["output"]
