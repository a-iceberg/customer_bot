...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        save_comment_tool = StructuredTool.from_function(
            ...
            name="Saving_comment",
            description="Сохраняет полезные по вашему мнению комментарии пользователя в заявку. Ни в коем случае НЕЛЬЗЯ передавать здесь информацию по по деталям адреса (квартира, подъезд и т.п.) или ЛЮБЫМ телефонам клиента, даже если он просит, в таком случае не используйте этото инструмент. Вам следует предоставить chat_id и непосредственно сам comment в качестве параметров.",
