...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        get_directions_tool = StructuredTool.from_function(
            ...
            description="Предоставляет актуальный перечень направлений обращения / ремонта. ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙТЕ после получения от пользователя причины, направления обращения и ДО использования инструмента сохранения направления для возможности соотнесения полученной информации с этим перечнем.",
            return_direct=False,
        )
