...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        request_tool = StructuredTool.from_function(
            ...
            description="Создает полностью заполненную заявку в 1С. Вам следует предоставить по отдельности сами значения ключей, соответствующих параметрам инструмента, словаря (request) с текущей заявкой из вашего системного промпта в качестве параметров инструмента, кроме ключа address_line_2. Из его же значения выделите и передайте отдельно при наличии непосредственно сами численно-буквенные значения apartment, entrance, floor и intercom (т.е. без слов) из всего address_line_2 в качестве остальных соответствующих параметров инструмента. Если какие-то из этих параметров не были предоставлены пользователем, передавайте их в инструмент со значением пустой строки - ''",
