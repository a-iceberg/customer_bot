...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        request_selection_tool = StructuredTool.from_function(
            ...
            description="Находит и предоставляет пользователю список его текущих заявок для выбора, чтобы определить контекст всего диалога, если речь идёт уже о каких-либо прошлых заявках, а не об оформлении новой, а номер заявки вам ещё неизвестен. Используйте ОБЯЗАТЕЛЬНО и ОДНОКРАТНО, СРАЗУ, когда вам нужно понять, о какой именно заявке идёт речь, например, когда пользователь снова хочет изменить или дополнить данные по существующей заявке. Если вы уже только что получили от пользователя номер заявки, повторно НИ В КОЕМ СЛУЧАЕ НЕ используйте этот инструмент! Вам следует предоставить chat_id в качестве параметра.",
