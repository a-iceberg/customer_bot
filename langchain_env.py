...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        get_directions_tool = StructuredTool.from_function(
            ...
            description="Предоставляет актуальный перечень направлений обращения / ремонта. ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙТЕ в начале диалога ДО приветсвия пользователя для возможности соотнесения последующей информации с этим перечнем.",
            return_direct=False,
        )
