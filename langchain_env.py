...
class ChatAgent:
    ...
    def initialize_agent(self):
        ...
        request_tool = StructuredTool.from_function(
            ...
            description="Создает полностью заполненную заявку в 1С. Вам следует предоставить просто значения ключей request в качестве параметров, кроме address_line_2. Из его значения выделите отдельно при наличии непосредственно сами значения apartment, entrance, floor и intercom без слов из всего address_line_2 в качестве остальных параметров. Если какие-то из этих параметров не были предоставлены пользователем, передавайте их в инструмент со значением пустой строки - ''",
